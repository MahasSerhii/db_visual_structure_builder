...
export const ContactModal: React.FC<ContactModalProps> = ({ isOpen, onClose }) => {
    ...
    const handleSend = () => {
        alert(`${t('contact.toast.sent')}\n${t('contact.label.email')}: ${email}\n${t('contact.label.message')}: ${message}`);
        onClose();
    };
        ...
        <div className="fixed inset-0 bg-gray-900 bg-opacity-50 z-50 flex items-center justify-center">
            <div className="bg-white rounded-2xl border border-gray-200 w-full max-w-md shadow-2xl overflow-hidden flex flex-col relative">
                <div className="p-6 border-b border-gray-100 flex-shrink-0 bg-white relative z-10 flex justify-between items-center">
                    <h2 className="text-2xl font-bold text-indigo-600">{t('contact.title')}</h2>
                ...
                <div className="p-6 space-y-4 bg-gray-50/50">
                     <div>
                        <label className="block text-sm font-semibold text-gray-600 mb-1">{t('contact.label.email')}</label>
                        <input 
                            type="email" 
                            value={email}
                     ...
                     <div>
                        <label className="block text-sm font-semibold text-gray-600 mb-1">{t('contact.label.message')}</label>
                        <textarea 
                            value={message}
